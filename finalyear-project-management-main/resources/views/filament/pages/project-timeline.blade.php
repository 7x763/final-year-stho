        ...
        <x-filament::section>
            <x-slot name="heading">
                <div class="flex items-center justify-between w-full">
                    <span>{{ __('Timeline View') }}</span>
                    <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                        <x-heroicon-o-eye class="w-4 h-4" />
                        <span>{{ __('Read Only Mode') }}</span>
                    </div>
                </div>
            </x-slot>
            ...
            <div class="w-full">
                ...
                @else
                    <div class="flex flex-col items-center justify-center h-64 text-gray-500 gap-4">
                        ...
                        <h3 class="text-lg font-medium">{{ __('No project data available') }}</h3>
                        <p class="text-sm">{{ __('Add start and end dates to projects to view timeline') }}</p>
                    </div>
                @endif
            </div>
        </x-filament::section>
        ...
        <x-filament::section>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('In Progress') }}</span>
                </div>
                <div class="flex items-center gap-2">
                    <div class="w-3 h-3 rounded" style="background-color: #10b981;"></div>
                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Nearly Complete') }}</span>
                </div>
                <div class="flex items-center gap-2">
                    <div class="w-3 h-3 rounded" style="background-color: #f59e0b;"></div>
                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Approaching Deadline') }}</span>
                </div>
                <div class="flex items-center gap-2">
                    <div class="w-3 h-3 rounded" style="background-color: #ef4444;"></div>
                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Overdue') }}</span>
                </div>
            </div>
        </x-filament::section>
    </div>
            ...
            function initializeGanttPage() {
                try {
                    ...
                    gantt.templates.tooltip_text = function(start, end, task) {
                        return `<b>{{ __('Project:') }}</b> ${task.text}<br/>
                                <b>{{ __('Status:') }}</b> ${task.status}<br/>
                                <b>{{ __('Duration:') }}</b> ${task.duration} {{ __('days') }}<br/>
                                <b>{{ __('Progress:') }}</b> ${Math.round(task.progress * 100)}%<br/>
                                <b>{{ __('Start:') }}</b> ${gantt.templates.tooltip_date_format(start)}<br/>
                                <b>{{ __('End:') }}</b> ${gantt.templates.tooltip_date_format(end)}
                                ${task.is_overdue ? '<br/><b style="color: #ef4444;">⚠️ {{ __('OVERDUE') }}</b>' : ''}`;
                    };
                    ...
                    if (container) {
                        container.innerHTML = `
                            <div class="flex flex-col items-center justify-center h-64 text-red-500 gap-4">
                                ...
                                <h3 class="text-lg font-medium">{{ __('Error loading timeline') }}</h3>
                                <p class="text-sm">{{ __('Please refresh the page or contact support') }}</p>
